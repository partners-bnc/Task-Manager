export function render(payload: Record<string, unknown>) {
  ...
  const employeeLines = employees.length > 0
    ? employees.map((emp: any) => `  - [${emp.employee_id || 'N/A'}] ${emp.name} (${emp.email})`).join('\n')
...
The HRM Attendance System`;
  ...
  <p style="font-weight: 600; margin-bottom: 8px; color: #000000;">Employees Missing Check-in:</p>
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td style="padding: 6px 0; color: #475569;">${emp.name}</td>
