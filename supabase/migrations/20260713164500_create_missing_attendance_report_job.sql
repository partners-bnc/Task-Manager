...
begin
  ...
  v_day_name := trim(to_char(v_report_date, 'Day')); -- e.g., 'Monday'

  -- Find active employees who:
  -- 1. Have today in their working_days array
  -- 2. Are not on leave today
  -- 3. Are not on holiday today
  -- 4. Have not checked in today (no hrm_attendance record with status not 'absent' or check_in not null)
  with missing as (
    select 
      e.employee_id,
      e.name,
    ...
    where e.employment_lifecycle_status = 'active'
      ...
      -- 4. Skip if employee is on approved leave today
      and not exists (
        select 1 
      ...
      -- 5. Must have NOT checked in yet (no row in hrm_attendance or check_in is null or status is absent)
      and not exists (
        select 1 
