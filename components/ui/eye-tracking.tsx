...
function Eye({
  ...
  const springX = useSpring(x, { stiffness: 800, damping: 35, mass: 0.3 });
  const springY = useSpring(y, { stiffness: 800, damping: 35, mass: 0.3 });
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      // Track over a wider screen radius of 800px so movement is progressive from near to far
      const trackingRadius = 800;
      const clampedDistance = Math.min(distance, trackingRadius);
      const normalizedDistance = clampedDistance / trackingRadius;
