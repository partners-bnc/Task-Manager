...
export default function VendorPayments() {
  ...
  const pagePayments = payments.filter((p) => p.payment_type === 'vendor_payment');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        const formData = new FormData();
        formData.append('payment_type', 'vendor_payment');
        formData.append('vendor_name', vendorName);
        formData.append('nature_of_payment', natureOfPayment);
        formData.append('amount', amount);
        formData.append('invoice_date', invoiceDate);
  ...
  const handleDeletePayment = async (paymentId) => {
    if (!confirm('Are you sure you want to delete this payment entry? This will also remove all attached files permanently.')) return;
    ...
    return (
      <div className="relative inline-block">
        <select
          ...
          className={`cursor-pointer rounded-full border px-3 py-1.5 text-[11px] font-extrabold outline-none transition-all pr-6.5 appearance-none bg-no-repeat bg-[right_6px_center] ${getStatusStyles(status)}`}
          style={{
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 border-b border-slate-200/80 pb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-extrabold tracking-tight text-slate-900 flex items-center gap-2.5">
            <div className="p-2.5 rounded-2xl bg-[linear-gradient(135deg,#e0f2fe_0%,#bae6fd_100%)] text-sky-700 shadow-xs shrink-0">
              <Receipt className="w-5 h-5" />
            </div>
            Vendor Payments
          </h1>
          <p className="text-slate-500 text-xs md:text-sm mt-1">
            Submit, review, and track regular payments and invoices generated by external vendors.
          </p>
        </div>
        ...
        <div className="flex bg-white p-1 rounded-2xl border border-slate-200/60 shadow-xs shrink-0 self-start lg:self-center">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-[11px] md:text-xs font-extrabold transition-all duration-200 ${
              activeTab === 'list'
                ? 'bg-[linear-gradient(180deg,#d7e7f9_0%,#7eb0ec_100%)] text-violet-950 shadow-xs'
                : 'text-slate-500 hover:text-slate-900'
            }`}
          >
            All Payments ({pagePayments.length})
          </button>
          <button
            onClick={() => {
              resetForm();
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
          <div className="overflow-x-auto w-full [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-300/70 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
            <table className="w-full text-left text-sm border-collapse min-w-[900px]">
              <thead>
                <tr className="border-b border-slate-100 text-[10px] md:text-[11px] font-extrabold uppercase tracking-wider text-slate-400 bg-slate-50/50">
                ...
                {pagePayments.length > 0 ? (
                  pagePayments.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-[11px] md:text-xs text-slate-705 font-bold">
                        {p.created_by_name}
                      </td>
                  ...
                  <tr>
                    <td colSpan={9} className="py-12 text-center text-slate-400 font-medium text-xs">
                      {"No vendor payments found. Click on 'Upload & Add New' to submit your first invoice."}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-5">
            ...
            <div>
              <h2 className="text-base md:text-lg font-bold text-slate-900">
                {editingId ? 'Edit Vendor Payment Entry' : 'New Vendor Payment Entry'}
              </h2>
              <p className="text-[11px] md:text-xs text-slate-400 mt-0.5">
                {editingId ? 'Modify payment details and update attachments.' : 'Submit payment details and attach related invoice/document files.'}
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Vendor Name *</label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nature of Payment *</label>
                <input
                  type="text"
                  required
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <File className={`w-4 h-4 flex-shrink-0 ${isDeleted ? 'text-rose-350' : 'text-blue-500'}`} />
                        ...
                        {isDeleted ? (
                          <button
                            ...
                            className="text-[11px] font-extrabold text-blue-600 hover:text-blue-750 underline transition-colors mr-1 cursor-pointer"
                          >
                            Undo
                          </button>
                        ) : (
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                {editingId ? 'Upload Additional Documents' : 'Upload Documents'}
              </label>
              
              <div className="border-2 border-dashed border-slate-200 rounded-3xl p-5 md:p-6 text-center hover:border-blue-500/50 transition-colors relative cursor-pointer group">
                <input
                  type="file"
                  multiple
                ...
                />
                <UploadCloud className="w-8 h-8 text-slate-400 mx-auto mb-2 group-hover:scale-105 transition-transform" />
                    ...
                    {selectedFiles.map((file, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-white p-2.5 rounded-xl border border-slate-200/50 shadow-xs">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-[11px] md:text-xs font-bold text-slate-800 truncate max-w-[140px]">{file.name}</p>
