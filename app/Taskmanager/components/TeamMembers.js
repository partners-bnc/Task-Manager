...
export default function TeamMembers() {
  ...
  const handleOpenAnalytics = (userId) => {
    ...
    router.push(`/Taskmanager/admin/team/${userId}`);
  };
  ...
  const openMemberDetails = async (userId) => {
    ...
    try {
      const response = await fetch(`/HRM/api/employees?id=${userId}`, { method: 'GET' });
  ...
  const handleDelete = async () => {
    ...
    setModalError('');
    try {
      const response = await fetch(`/HRM/api/employees?id=${selectedUser.id}`, {
        method: 'DELETE',
      });
      const result = await response.json();
