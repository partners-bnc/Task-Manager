...
export default function TeamMembers() {
	const { users, tasks, loading, error, refreshData } = useData();
				...
				{users.map((user) => {
					...
					return (
						<div key={user.id} className="relative bg-white p-6 rounded-xl shadow-sm flex flex-col items-center text-center">
							...
							<div className="w-20 h-20 rounded-full overflow-hidden mb-4 border-4 border-[#7F40EE]/10 flex items-center justify-center bg-slate-200 text-slate-700 text-xl font-semibold">
								{avatarSrc ? (
									<Image src={avatarSrc} alt={user.name} width={80} height={80} className="w-full h-full object-cover" />
								) : (
