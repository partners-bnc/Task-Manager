...
async function rollupAttendanceForDay(employeeId, attendanceDate, attendanceId) {
  ...
  console.log("Updating database with payload:", JSON.stringify(attendancePayload, null, 2));
...
async function run() {
  try {
    ...
    const attendanceId = '50b6dc0f-0168-437f-8258-96f882f4a683';

    // Print initial row in DB
    const { data: initialRow } = await supabase
      .from('hrm_attendance')
      .select('*')
      .eq('id', attendanceId)
      .single();
    console.log("Initial DB status:", initialRow.status);

    const result = await rollupAttendanceForDay(employeeId, date, attendanceId);
    console.log("Updated DB row returned:", JSON.stringify(result, null, 2));
    ...
    const { data: finalRow } = await supabase
      ...
      .eq('id', attendanceId)
      .single();
    console.log("Final DB status fetched again:", finalRow.status);
  } catch (err) {
    console.error(err);
  }
}
