...
const normalizeTask = (task, fallbackAssignees = []) => {
...
export function DataProvider({ children, initialUser = null, mode = 'employee' }) {
  ...
  const fetchAdminData = async () => {
    ...
    const nextTasks = (tasksJson.tasks || []).map((task) => normalizeTask(task));
  ...
  const fetchEmployeeData = async () => {
    ...
    const nextTasks = rawTasks.map((task) => normalizeTask(task, employee?.id ? [employee.id] : []));
